...
void parlay_merge(slice<InIterator, InIterator> A, slice<TmpIterator, TmpIterator> Tmp, const GetKey &g, size_t n1,
                  size_t n2) {
  using in_type = typename slice<InIterator, InIterator>::value_type;
  internal::merge_into<uninitialized_relocate_tag>(A.cut(0, n1), A.cut(n1, n1 + n2), Tmp.cut(0, n1 + n2),
                                                   [&](const in_type &a, const in_type &b) { return g(a) < g(b); });
  uninitialized_relocate(Tmp.begin(), Tmp.begin() + n1 + n2, A.begin());
}

template<typename InIterator, typename TmpIterator, typename DistIterator, typename OffsetIterator, typename GetKey>
void merge_out_of_place(slice<InIterator, InIterator> A, slice<TmpIterator, TmpIterator> Tmp,
                        slice<DistIterator, DistIterator> dist, slice<OffsetIterator, OffsetIterator> offsets,
  ...
  for (size_t i = 0; i < num_dist; i++) {
    ...
    heavy_offsets[i + 1] = std::lower_bound(get_key.begin(), get_key.end(), k) - get_key.begin();
  ...
  if (n1 + n2 < MERGE_BASE_CASE_SIZE) {
    ...
    for (size_t i = 0; i < num_dist + 1; i++) {
      uninitialized_relocate(A.begin() + heavy_offsets[i], A.begin() + heavy_offsets[i + 1],
                             Tmp.begin() + heavy_counts[i]);
    }
    // move heavy keys
    for (size_t i = 0; i < num_dist; i++) {
      uninitialized_relocate(A.begin() + n1 + heavy_counts[i], A.begin() + n1 + heavy_counts[i + 1],
...
template<typename InIterator, typename TmpIterator, typename DistIterator, typename OffsetIterator, typename GetKey>
void merge_inplace(slice<InIterator, InIterator> A, slice<TmpIterator, TmpIterator> Tmp,
                   slice<DistIterator, DistIterator> dist, slice<OffsetIterator, OffsetIterator> offsets,
                   const GetKey &g, size_t n1, size_t n2) {
  assert(n1 + n2 == A.size());
  using in_type = typename slice<InIterator, InIterator>::value_type;
  ...
  if (n1 + n2 < MERGE_BASE_CASE_SIZE) {
    if (n1 >= n2) {
      ...
      for (size_t i = num_dist; i >= 1; i--) {
        for (ptrdiff_t j = heavy_offsets[i + 1] - 1; j >= (ptrdiff_t)heavy_offsets[i]; j--) {
          assign_dispatch(A[heavy_counts[i] + j], A[j], uninitialized_relocate_tag());
        }
      }
      // move heavy keys
      for (size_t i = 0; i < num_dist; i++) {
        uninitialized_relocate(Tmp.begin() + heavy_counts[i], Tmp.begin() + heavy_counts[i + 1],
      ...
      for (size_t i = 0; i < num_dist; i++) {
        for (size_t j = heavy_counts[i]; j < heavy_counts[i + 1]; j++) {
          assign_dispatch(A[heavy_offsets[i + 1] + j], A[n1 + j], uninitialized_relocate_tag());
        }
      }
      // move light keys
      for (size_t i = 0; i < num_dist + 1; i++) {
        uninitialized_relocate(Tmp.begin() + heavy_offsets[i], Tmp.begin() + heavy_offsets[i + 1],
  ...
  if (n1 >= n2) {
    ...
    for (size_t i = num_dist; i >= 1; i--) {
      if (heavy_offsets[i + 1] <= heavy_counts[i] + heavy_offsets[i]) {
        ...
        reverse_inplace(A.cut(heavy_offsets[i] + heavy_counts[i], heavy_offsets[i + 1]));
        size_t size = heavy_counts[i];
        parallel_for(0, size, [&](size_t j) {
          assign_dispatch(A[heavy_counts[i] + heavy_offsets[i + 1] - 1 - j], A[heavy_offsets[i] + j],
    ...
    for (size_t i = 0; i < num_dist; i++) {
      if (heavy_offsets[i + 1] + heavy_counts[i + 1] <= n1 + heavy_counts[i]) {
        ...
        reverse_inplace(A.cut(n1 + heavy_counts[i], heavy_offsets[i + 1] + heavy_counts[i + 1]));
        size_t size = n1 - heavy_offsets[i + 1];
        parallel_for(0, size, [&](size_t j) {
          assign_dispatch(A[n1 + heavy_counts[i] - 1 - j], A[heavy_offsets[i + 1] + heavy_counts[i + 1] + j],
                          uninitialized_relocate_tag());
        });
      }
    }
...
template<typename s_size_t, typename inplace_tag, typename assignment_tag, typename InIterator, typename OutIterator,
         typename TmpIterator, typename GetKey>
void integer_sort2_(slice<InIterator, InIterator> In, slice<OutIterator, OutIterator> Out,
                    slice<TmpIterator, TmpIterator> Tmp, const GetKey &g, size_t key_bits, double parallelism = 1.0) {
  ...
  if (n < INTEGER_SORT_BASE_CASE_SIZE || parallelism < .0001) {
    internal::seq_radix_sort<inplace_tag, assignment_tag>(In, Out, Tmp, g, key_bits);
    return;
  }
  ...
  internal::seq_sort_inplace(make_slice(samples), [&](const key_type &a, const key_type &b) { return a < b; }, false);
  ...
  if (top_bits <= log2_light_keys) {
    ...
    std::tie(bucket_offsets, one_bucket) = internal::count_sort_<assignment_tag, s_size_t>(
        In, Out, make_slice(get_bits), num_buckets, parallelism, inplace_tag::value);
    ...
    if (bucket_offsets[light_buckets] != bucket_offsets[light_buckets + 1]) {
      ...
      auto a = Arr.cut(bucket_offsets[light_buckets], bucket_offsets[light_buckets + 1]);
      internal::seq_sort_inplace(a, [&](const in_type &i1, const in_type &i2) { return g(i1) < g(i2); }, true);
    }
    return;
  }
  ...
  sequence<std::pair<key_type, BktId>> heavy_id(heavy_id_size, {key_type{}, SMAX});
  ...
  size_t num_buckets = heavy_buckets + light_buckets + 1;  // with an extra overflow buckets
  ...
  if constexpr (inplace_tag::value == true) {
    parallel_for(0, heavy_buckets, [&](size_t i) {
      ...
      uninitialized_relocate(Out.begin() + start, Out.begin() + end, In.begin() + start);
    });
  }
  ...
  for (size_t i = 0; i < light_buckets + 1; i++) {
    total_elements += bucket_offsets[light_id[i] + 1] - bucket_offsets[light_id[i]];
  }
  ...
  for (size_t i = 0; i < light_buckets + 1; i++) {
    ...
    cumulative_elements += bucket_offsets[light_id[i] + 1] - bucket_offsets[light_id[i]];
  }
      ...
      [&](size_t i) {
        ...
        for (size_t j = block_s; j < block_e; j++) {
          ...
          if (start != end) {
            ...
            if (j != light_buckets) {
              integer_sort2_<s_size_t, typename std::negation<inplace_tag>::type, uninitialized_relocate_tag>(
                  a, b, a, g, top_bits - log2_light_keys, (parallelism * (end - start)) / (total_elements + 1));
              size_t n1 = bucket_offsets[light_id[j] + 1] - bucket_offsets[light_id[j]];
              size_t n2 = bucket_offsets[light_id[j + 1]] - bucket_offsets[light_id[j] + 1];
              if (n1 != 0 && n2 != 0) {
                size_t s_seq = light_id[j] - j;
                size_t t_seq = light_id[j + 1] - (j + 1);
                merge_inplace(Arr.cut(start, start + n1 + n2), Tmp2.cut(start, start + n1 + n2),
                              heavy_seq.cut(s_seq, t_seq), bucket_offsets.cut(light_id[j] + 1, light_id[j + 1]), g, n1,
                              n2);
              }
            } else {
              if constexpr (std::negation<inplace_tag>::value == true) {
                internal::seq_sort_inplace(
                    a, [&](const in_type &i1, const in_type &i2) { return g(i1) < g(i2); }, true);
              } else {
                internal::seq_sort_<uninitialized_relocate_tag>(
                    a, b, [&](const in_type &i1, const in_type &i2) { return g(i1) < g(i2); }, true);
              }
            }
          }
        }
...
auto integer_sort2(slice<Iterator, Iterator> In, const GetKey &g) {
  ...
  using key_type = typename std::invoke_result<GetKey, typename slice<Iterator, Iterator>::value_type>::type;
  if (In.size() < max32) {
    integer_sort2_<uint32_t, std::false_type, uninitialized_copy_tag>(In, make_slice(Out), make_slice(Tmp), g,
                                                                      sizeof(key_type) * 8);
  } else {
    integer_sort2_<size_t, std::false_type, uninitialized_copy_tag>(In, make_slice(Out), make_slice(Tmp), g,
                                                                    sizeof(key_type) * 8);
  }
  return Out;
}
...
void integer_sort_inplace2(slice<Iterator, Iterator> In, const GetKey &g) {
  ...
  using key_type = typename std::invoke_result<GetKey, typename slice<Iterator, Iterator>::value_type>::type;
  if (In.size() < max32) {
    integer_sort2_<uint32_t, std::true_type, uninitialized_relocate_tag>(In, make_slice(Tmp), In, g,
                                                                         sizeof(key_type) * 8);
  } else {
    integer_sort2_<size_t, std::true_type, uninitialized_relocate_tag>(In, make_slice(Tmp), In, g,
                                                                       sizeof(key_type) * 8);
  }
}
